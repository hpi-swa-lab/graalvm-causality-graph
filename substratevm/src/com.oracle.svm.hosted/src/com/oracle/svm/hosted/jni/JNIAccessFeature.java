...
public class JNIAccessFeature implements Feature {
    ...
    private class JNIRuntimeAccessibilitySupportImpl extends ConditionalConfigurationRegistry
        ...
        public void register(ConfigurationCondition condition, boolean unsafeAllocated, Class<?> clazz) {
            ...
            registerConditionalConfiguration(condition, (cnd) -> {
                CausalityExport.registerEvent(Facts.JNIRegistration.create(clazz));
            ...
            if (!queriedOnly) {
                registerConditionalConfiguration(condition, (cnd) -> {
                    for (Executable m : executables) {
                        CausalityExport.registerEvent(Facts.JNIRegistration.create(m));
                    }
        ...
        private void registerFields(boolean finalIsWritable, Field[] fields) {
            for (Field field : fields) {
                CausalityExport.registerEvent(Facts.JNIRegistration.create(field));
    ...
    public void duringAnalysis(DuringAnalysisAccess a) {
        ...
        for (Class<?> clazz : newClasses) {
            try (var ignored = CausalityExport.setCause(Facts.JNIRegistration.create(clazz))) {
        ...
        for (Executable method : newMethods) {
            try (var ignored = CausalityExport.setCause(Facts.JNIRegistration.create(method))) {
        ...
        newFields.forEach((field, writable) -> {
            try (var ignored = CausalityExport.setCause(Facts.JNIRegistration.create(field))) {
    ...
    private JNIJavaCallVariantWrapperGroup createJavaCallVariantWrappers(DuringAnalysisAccessImpl access, ResolvedSignature<ResolvedJavaType> wrapperSignature, boolean nonVirtual) {
        ...
        CausalityExport.registerEvent(Facts.JniCallVariantWrapper.create(wrapperSignature, !nonVirtual));
        return map.computeIfAbsent(wrapperSignature, signature -> {
            try (var ignored = CausalityExport.overwriteCause(Facts.JniCallVariantWrapper.create(wrapperSignature, !nonVirtual))) {
